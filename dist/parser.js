...
break;
case 19:
 yy.debug('wut', $$[$0-3], $$[$0-2], $$[$0-1], $$[$0]); this.$ = yy.datediff($$[$0-3], $$[$0-2], $$[$0-1], $$[$0]) 
break;
case 20:
 this.$ = yy.unescape($$[$0]); 
break;
...
case 7:return 17;
break;
case 8:this.pushState('reldate'); return 24;
break;
case 9:return 31;
break;
case 10:return 30;
...
case 11:return 26
break;
case 12:this.popState(); return 27
break;
case 13:return 28;
break;
case 14:return 29;
...
rules: [/^(?:\s+)/,/^(?:(?:null|NULL|Null)(?!(\\(['"\+\,\(\)\>\<=\[\]])|([^\s'"\+\,\(\)\>\<=\[\]]))+))/,/^(?:(?:true|TRUE|True)(?!(\\(['"\+\,\(\)\>\<=\[\]])|([^\s'"\+\,\(\)\>\<=\[\]]))+))/,/^(?:(?:false|FALSE|False)(?!(\\(['"\+\,\(\)\>\<=\[\]])|([^\s'"\+\,\(\)\>\<=\[\]]))+))/,/^(?:[a-zA-Z_][a-zA-Z0-9_\.]*[:])/,/^(?:[0-9]+(\.[0-9]+)?\b(?![\-]))/,/^(?:\[)/,/^(?:\])/,/^(?:now(?=[-+]\d+[dwMy]))/,/^(?:-)/,/^(?:\+)/,/^(?:\d+)/,/^(?:[dwMy])/,/^(?:([^\s'"\+\,\(\)\>\<=\[\]\-])(\\(['"\+\,\(\)\>\<=\[\]])|([^\s'"\+\,\(\)\>\<=\[\]]))+)/,/^(?:['](\\['"]|[^'"])+?['])/,/^(?:\()/,/^(?:\))/,/^(?:,)/,/^(?:\+)/,/^(?:-)/,/^(?:>=)/,/^(?:<=)/,/^(?:>)/,/^(?:<)/],
